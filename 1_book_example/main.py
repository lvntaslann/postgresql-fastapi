...
def get_book_by_id(id:int,db:Session=Depends(get_db)):
    book_query_set = services.get_book(db,id)
    if book_query_set:
...
def update_book(book:schemas.BookCreate,id:int,db:Session = Depends(get_db)):
    ...
    if not db_update:
        raise HTTPException(status_cpde = 404 , detail = "Book not found")
    return db_update
...
def delete_book(id:int,db:Session = Depends(get_db)):
    ...
    if delete_entry:
        return delete_entry
    raise HTTPException(statuscode = 404,detail="Book not found")
